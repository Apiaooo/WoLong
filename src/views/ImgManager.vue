<template>
  <div>
    <div class="imgtb">
      <el-table v-loading="loading" :data="tableData" height="600" style="width: 100%;">
        ...
        <el-table-column label="图片" prop="fileName" align="center">
          <template slot-scope="scope">
            <img :src="url + scope.row.fileName" style="width: 80px;height:80px" />
          </template>
        </el-table-column>
      ...
      <div style="color:red">温馨提示：8张图片显示效果最佳</div>
    </div>
    <div class="bt">
    ...
    <el-dialog title="上传图片" :visible.sync="dialogFormVisible">
      <div class="imgBox">
        <el-upload
          action="http://47.115.143.135:8080/img/upload"
...
export default {
  ...
  methods: {
    ...
    deleteImg(row) {
      const that = this;
      axios
        .delete("http://47.115.143.135:8080/img/deleteById/" + row.id)
        .then(function(resp) {
          that.$message({
            message: "删除成功！",
            type: "success"
